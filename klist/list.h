...
/* prefetch on hash list is harmful */
static inline int list_prefetch(void *x)
{
	return 0;
}
...
#define list_for_each_rcu(pos, head) \
	for (pos = (head)->next; \
		list_prefetch(rcu_dereference(pos)->next), pos != (head); \
        	pos = pos->next)
...
#define list_for_each_entry_rcu(pos, head, member) \
	for (pos = list_entry((head)->next, typeof(*pos), member); \
		list_prefetch(rcu_dereference(pos)->member.next), \
			&pos->member != (head); \
		pos = list_entry(pos->member.next, typeof(*pos), member))
...
#define hlist_for_each_entry_rcu(tpos, pos, head, member)		 \
	for (pos = (head)->first;					 \
	     rcu_dereference(pos) && ({ list_prefetch(pos->next); 1;}) &&	 \
		({ tpos = hlist_entry(pos, typeof(*tpos), member); 1;}); \
	     pos = pos->next)
