...
void *do_cache_alloc(cache_t *cache)
{
	...
	if (cache->freecurr > 0) {
		...
		if (ret) {
			object = get_object(ret);
			if (cache->constructor && cache->constructor(object, NULL, 0) != 0) {
				free(ret);
				object = NULL;
			}
			cache->total++;
...
void do_cache_free(cache_t *cache, void *ptr)
{
#ifndef NDEBUG
	/* validate redzone... */
	if (memcmp((char *)ptr + cache->bufsize - (2 * sizeof(redzone_pattern)), &redzone_pattern, sizeof(redzone_pattern)) != 0) {
		raise(SIGABRT);
		cache_error = 1;
		return ;
	}
	...
	if (cache->limit != 0 && cache->limit < cache->total) {
		...
		size_t newtotal = 2 * cache->freetotal;
